...
# Margins and legend
set bmargin 6
set key at screen 0.2,0.04 Left reverse spacing 1.2 width -10 height 1
...
for ((i = cardInit; i < 10; i++)); do
    if [[ -n "${card_names[$i]}" ]]; then
        ...
        echo "'/tmp/card${i}_data.txt' using 1:2 title 'GPU%' with lines lw 2 lc rgb '${color}', \\" >> /tmp/plot.gnu
